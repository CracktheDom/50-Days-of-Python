...
def check_duplicates(str_list:list) -> str:
	duplicates = []
	for i in range(len(str_list)):
		if str_list.count(str_list[i]) >= 2 and str_list[i] not in duplicates:
			duplicates.append(str_list[i])
		elif str_list.count(str_list[i]) < 2:
			continue
...
print(check_duplicates(['apples', 'banana', 'guava', 'banana', 'lemon', 'lime', 'kiwi', 'dragonfruit', 'mango', 'lime']))